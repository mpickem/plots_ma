...
asy = h5py.File('../Output-u50-wo-gamma-newgiw/adga-20180126-013536.333-output.hdf5','r')
...
for i in xrange(6):
  for j in xrange(i,6):
    ...
    if (i==3 or i==0):
      cb1 = plt.colorbar(ticks=[a,a+(b-a)/3,a+(b-a)*2/3, b], format='%.0f', shrink=0.650)
    else:
...
plt.savefig('layer-ins-3x6-susc-magn.eps', bbox_inches='tight', format='eps', dpi=600, Rasterized=True)
plt.show()
