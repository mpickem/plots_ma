...
a = gamma_plt[:-1,:-1].min()
b = gamma_plt[:-1,:-1].max()
cb3 = plt.colorbar(p1, ticks=[a,a+(b-a)/3,a+(b-a)*2/3, b], format='%.2f', shrink=0.715)
...
plt.tight_layout()
plt.savefig('layer-contour-bot.eps', bbox_inches='tight', format='eps', dpi=600, Rasterized=True)
plt.show()
